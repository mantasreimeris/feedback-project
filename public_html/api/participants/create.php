...
function form_success($filtered_input, $form) {
    ...
    $person_feedback = new \App\Feedbacks\Feedback($filtered_input);
    $model = new \App\Feedbacks\Model();
    $id = $model->insert($person_feedback);

    if ($id !== false) {
        $person_feedback->setId($id);
        $response->setData($person_feedback->getData());
    } else {
